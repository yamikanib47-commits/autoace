  ...
  try {
    ...
    const row = {
      ...
      photo_paths: Array.isArray(input.photo_paths)
        ? input.photo_paths.filter((item): item is string => typeof item === "string").slice(0, 20)
        : [],
    ...
    if (row.buyer_request_id) {
      const matchResult = await supabase.from("matches").upsert({
        buyer_request_id: row.buyer_request_id,
        vehicle_listing_id: listingId,
      }, { onConflict: "buyer_request_id,vehicle_listing_id", ignoreDuplicates: true });
    ...
    return jsonResponse({
      status: "error",
      message: error instanceof Error ? error.message : "Seller submission failed",
    }, 400);
  }
});
