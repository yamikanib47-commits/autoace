...
Deno.serve(async (request) => {
  ...
  if (request.method !== "GET" && request.method !== "POST") return jsonResponse({ status: "error", message: "Method not allowed" }, 405);
  try {
    const input = request.method === "GET" ? Object.fromEntries(new URL(request.url).searchParams) : await request.json();
    let query = getSupabaseAdmin().from("public_vehicle_listings").select("*").order("created_at", { ascending: false });
    ...
    return jsonResponse({ status: "error", message: error instanceof Error ? error.message : "Vehicle search failed" }, 400);
  }
});
