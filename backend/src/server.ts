...
const asMatch = (r: Row) => ({ id: r.id, buyerRequestId: r.buyer_request_id, vehicleListingId: r.vehicle_listing_id, status: r.status, adminNotes: r.admin_notes, createdAt: r.created_at, updatedAt: r.updated_at, buyer: r.buyer_requests ? asBuyer(r.buyer_requests) : Array.isArray(r.buyer_requests) && r.buyer_requests[0] ? asBuyer(r.buyer_requests[0]) : undefined, vehicle: r.vehicle_listings ? asVehicle(r.vehicle_listings) : Array.isArray(r.vehicle_listings) && r.vehicle_listings[0] ? asVehicle(r.vehicle_listings[0]) : undefined });
...
app.post("/auth/sign-out", async (c) => { await requireUser(c); return c.body(null, 204); });
...
app.delete("/buyer-requests/:id", async (c) => { await requireAdmin(c); const { error } = await supabase.from("buyer_requests").delete().eq("id", c.req.param("id")); if (error) throw new HTTPException(400, { message: error.message }); return c.body(null, 204); });
...
app.delete("/vehicles/:id", async (c) => { await requireAdmin(c); const { error } = await supabase.from("vehicle_listings").delete().eq("id", c.req.param("id")); if (error) throw new HTTPException(400, { message: error.message }); return c.body(null, 204); });
...
app.post("/storage/vehicle-photos/resolve", async (c) => { const input = await body(c, z.object({ paths: z.array(z.string().max(300)).max(30) })); const urls = input.paths.map((path) => supabase.storage.from("vehicle-photos").getPublicUrl(path).data.publicUrl); return c.json({ urls }); });
